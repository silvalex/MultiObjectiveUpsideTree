...
public class WSCMultiObjectiveStatistics extends MultiObjectiveStatistics {
	...
	public void postEvaluationStatistics(final EvolutionState state) {
		...
		for (int x = 0; x < state.population.subpops.length; x++) {
			for (int y = 0; y < state.population.subpops[x].individuals.length; y++) {
				TreeIndividual ind = (TreeIndividual) state.population.subpops[x].individuals[y];
		...
		if (!silentFront) {
			...
			for (int s = 0; s < state.population.subpops.length; s++) {
				...
				for (int i = 0; i < sortedFront.length; i++) {
					TreeIndividual ind = (TreeIndividual) sortedFront[i];
					StringBuilder builder = new StringBuilder();
	 ...
	 * @param finalFront
	 */
	public void individualStringRepresentation( TreeIndividual ind, StringBuilder builder, boolean finalFront) {
